...
var REVIEWS_FILE = import_path.default.join(process.cwd(), "reviews.json");
var SETTINGS_FILE = import_path.default.join(process.cwd(), "settings.json");
var defaultPaymobSettings = {
...
var defaultEmailVerificationSettings = {
  provider: process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD ? "gmail" : "firebase_auth",
  gmailUser: process.env.GMAIL_USER || "",
  gmailAppPassword: process.env.GMAIL_APP_PASSWORD || ""
};
function loadSettings() {
  const envSheetUrl = process.env.GOOGLE_SHEET_WEBHOOK_URL && process.env.GOOGLE_SHEET_WEBHOOK_URL.startsWith("http") ? process.env.GOOGLE_SHEET_WEBHOOK_URL.trim() : "";
  try {
    if (import_fs.default.existsSync(SETTINGS_FILE)) {
      const data = import_fs.default.readFileSync(SETTINGS_FILE, "utf-8");
      const parsed = JSON.parse(data);
      return {
        googleSheetWebhookUrl: parsed.googleSheetWebhookUrl && parsed.googleSheetWebhookUrl.startsWith("http") ? parsed.googleSheetWebhookUrl.trim() : envSheetUrl,
        paymob: {
...
function loadReviews() {
  try {
    if (import_fs.default.existsSync(REVIEWS_FILE)) {
      const data = import_fs.default.readFileSync(REVIEWS_FILE, "utf-8");
      const parsed = JSON.parse(data);
      if (Array.isArray(parsed)) return parsed;
    }
  } catch (e) {
...
var currentReviews = loadReviews();
var currentSettings = loadSettings();
var dispatchedSheetOrders = /* @__PURE__ */ new Set();
var otpStore = /* @__PURE__ */ new Map();
var otpRateLimit = /* @__PURE__ */ new Map();
var verifiedTokens = /* @__PURE__ */ new Map();
setInterval(() => {
  const now = Date.now();
  ...
  for (const [key, rl] of otpRateLimit.entries()) {
    if (now - rl.windowStart > 10 * 60 * 1e3) {
      otpRateLimit.delete(key);
    }
  }
}, 5 * 60 * 1e3);
async function forwardOrderToGoogleSheet(order, clientWebhookUrl, force = false) {
  ...
  const envSheet = process.env.GOOGLE_SHEET_WEBHOOK_URL && process.env.GOOGLE_SHEET_WEBHOOK_URL.startsWith("http") ? process.env.GOOGLE_SHEET_WEBHOOK_URL.trim() : "";
  const webhookUrl = clientWebhookUrl && clientWebhookUrl.startsWith("http") ? clientWebhookUrl.trim() : currentSettings.googleSheetWebhookUrl && currentSettings.googleSheetWebhookUrl.startsWith("http") ? currentSettings.googleSheetWebhookUrl.trim() : envSheet;
  if (!webhookUrl) {
  ...
  const itemsText = (order.items || []).map((i) => `${i.name}${i.details ? ` (${i.details})` : ""} - ${i.price} LE`).join(" | ");
  ...
  const payload = {
    ...
    date: order.date || (/* @__PURE__ */ new Date()).toLocaleDateString(),
  ...
  try {
    ...
    urlObj.searchParams.set("date", order.date || (/* @__PURE__ */ new Date()).toLocaleDateString());
  ...
  while (attempt <= 2) {
    try {
      ...
      const isRedirectedToAuth = res.url && res.url.includes("accounts.google.com") || responseText.includes("You need access") || responseText.includes("accounts.google.com") || responseText.includes("Sign in - Google Accounts");
...
async function forwardReviewToGoogleSheet(review) {
...
async function startServer() {
  ...
  app.get("/api/orders/:orderNumber", (req, res) => {
    ...
    const order = currentOrders.find((o) => {
      ...
      return oNum === cleanNum || oNum === `SST-${cleanNum}` || oNum.includes(cleanNum) || oId === cleanNum;
    });
    if (order) {
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      const newOrder = {
        ...
        date: body.date || (/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" }),
        items: body.items || [],
        total: body.total,
  ...
  const orderStreamClients = /* @__PURE__ */ new Set();
  function broadcastOrdersUpdate(updatedOrder) {
...
`);
  ...
  const handleOrderStatusUpdate = (req, res) => {
    try {
      ...
      currentOrders = currentOrders.map((ord) => {
        if (ord.orderNumber === orderNumber || ord.id === orderNumber || ord.orderNumber?.toLowerCase() === orderNumber?.toLowerCase()) {
          updated = true;
  ...
  app.post("/api/webhook/order-status", (req, res) => {
    try {
      const body = req.body || {};
      const orderIdentifier = body.orderNumber || body.order || body.Order || body.orderId || body.order_id || body.Number || body.number;
      ...
      currentOrders = currentOrders.map((ord) => {
        if (ord.orderNumber === orderIdentifier || ord.id === orderIdentifier || ord.orderNumber?.toLowerCase() === String(orderIdentifier).toLowerCase()) {
          updated = true;
  ...
  const handleOrderRating = (req, res) => {
    try {
      const { orderNumber } = req.params;
      ...
      const ratingTimestamp = ratedAt || (/* @__PURE__ */ new Date()).toISOString();
      currentOrders = currentOrders.map((ord) => {
        if (ord.orderNumber === orderNumber || ord.id === orderNumber || ord.orderNumber?.toLowerCase() === orderNumber?.toLowerCase()) {
          updated = true;
          matchedOrder = {
  ...
  app.get("/api/orders/export-excel", (req, res) => {
    try {
      ...
      res.setHeader("Content-Disposition", `attachment; filename="medgastore-orders-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.csv"`);
  ...
  app.post("/api/settings/google-sheets", async (req, res) => {
    try {
      ...
      if (testOrder && currentSettings.googleSheetWebhookUrl) {
        const targetOrder = currentOrders.find((o) => !o.orderNumber?.includes("TEST")) || currentOrders[0];
        if (targetOrder) {
    ...
    try {
      ...
      if (requestedOrderNumber) {
        targetOrder = currentOrders.find(
          (o) => o.orderNumber === requestedOrderNumber || o.id === requestedOrderNumber
        );
      }
      if (!targetOrder && currentOrders.length > 0) {
        targetOrder = currentOrders.find((o) => !o.orderNumber?.includes("TEST")) || currentOrders[0];
      }
      if (!targetOrder) {
        return res.status(400).json({
          success: false,
      ...
      return res.status(400).json({
        success: false,
        is403,
        error: is403 ? "HTTP 403 Forbidden / Access Denied: Google redirected to sign-in. In Google Apps Script, click Deploy > Manage deployments > Edit > set 'Who has access' to 'Anyone' (not 'Only myself')." : `Failed: ${testResult.error}`
      });
    } catch (err) {
  ...
  app.post("/api/settings/google-sheets/sync-all", async (req, res) => {
    try {
      ...
      const realOrders = currentOrders.filter(
        (o) => !o.orderNumber?.includes("TEST") && !o.orderNumber?.includes("VERIFY")
      );
  ...
  app.post("/api/settings/google-sheets/send-order", async (req, res) => {
    try {
      ...
      const targetOrder = currentOrders.find((o) => o.orderNumber === orderNumber || o.id === orderNumber);
      if (!targetOrder) {
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: Arial, sans-serif; background-color: #0f172a; color: #f8fafc; padding: 24px; border-radius: 12px; max-width: 480px; margin: auto;">
            <h3 style="color: #a855f7; margin-top: 0;">\u{1F3AE} \u0645\u062A\u062C\u0631 \u0645\u064A\u062F\u062C\u0627 - \u0627\u062E\u062A\u0628\u0627\u0631 \u0625\u0631\u0633\u0627\u0644 \u0627\u0644\u0628\u0631\u064A\u062F \u0628\u0646\u062C\u0627\u062D!</h3>
            ...
            <p style="color: #94a3b8; font-size: 12px;">\u062A\u0627\u0631\u064A\u062E \u0627\u0644\u0627\u062E\u062A\u0628\u0627\u0631: ${(/* @__PURE__ */ new Date()).toLocaleString("ar-EG")}</p>
          </div>
        `
      });
      return res.json({
  ...
  const processedPaymobWebhooks = /* @__PURE__ */ new Set();
  app.post("/api/paymob/callback", async (req, res) => {
    try {
      ...
      if (hmacSecret && req.query.hmac) {
        ...
        const calculatedHmac = import_crypto.default.createHmac("sha512", hmacSecret).update(concatenated).digest("hex");
  ...
  app.post("/api/email-verify/confirm-firebase", (req, res) => {
    ...
    verifiedTokens.set(verificationToken, { target: cleanEmail, expiresAt: Date.now() + 60 * 60 * 1e3 });
  ...
  app.post("/api/otp/send", async (req, res) => {
    try {
      ...
      if (rlEntry && now - rlEntry.windowStart < 10 * 60 * 1e3) {
        if (rlEntry.count >= 3) {
      ...
      if (gmailUser && gmailPass) {
        ...
        const codeHash = import_crypto.default.createHmac("sha256", salt).update(code).digest("hex");
        ...
        await transporter.sendMail({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; background-color: #0f172a; color: #f8fafc; padding: 28px; border-radius: 16px; max-width: 500px; margin: auto;">
              <h2 style="color: #a855f7; margin-top: 0; font-size: 22px;">\u{1F3AE} \u0645\u062A\u062C\u0631 \u0645\u064A\u062F\u062C\u0627 \u0644\u0644\u0623\u0644\u0639\u0627\u0628 - \u062A\u0623\u0643\u064A\u062F \u0627\u0644\u0628\u0631\u064A\u062F</h2>
  ...
  app.post("/api/otp/verify", (req, res) => {
    try {
      ...
      const inputHash = import_crypto.default.createHmac("sha256", stored.salt).update(cleanCode).digest("hex");
      const hashBufferA = Buffer.from(inputHash, "hex");
      const hashBufferB = Buffer.from(stored.codeHash, "hex");
      if (hashBufferA.length !== hashBufferB.length || !import_crypto.default.timingSafeEqual(hashBufferA, hashBufferB)) {
        stored.attempts += 1;
      ...
      otpStore.delete(cleanTarget);
      const verificationToken = import_crypto.default.randomBytes(32).toString("hex");
      verifiedTokens.set(verificationToken, { target: cleanTarget, expiresAt: now + 30 * 60 * 1e3 });
  ...
  const handleAiChat = async (req, res) => {
    try {
      ...
      const cleanAiReply = (text) => {
        if (!text) return "";
        return text.replace(/\*{2,}/g, "").replace(/^(\s*)\*\s+/gm, "$1\u2022 ").trim();
      };
      const getFallbackReply = () => {
        ...
        if (lastUserMsg.includes("fc") || lastUserMsg.includes("fifa") || lastUserMsg.includes("gta") || lastUserMsg.includes("v-bucks") || lastUserMsg.includes("vbucks") || lastUserMsg.includes("game") || lastUserMsg.includes("\u0644\u0639\u0628")) {
          return "\u{1F3AE} Top PlayStation Games & Offers:\n\u2022 EA Sports FC 26: 1,000 EGP\n\u2022 GTA VI Pre-order: 1,500 EGP\n\u2022 Fortnite V-Bucks: starting at 300 EGP for 1,000 V-Bucks\n\u2022 PS Plus Subscriptions: Essential, Extra, & Deluxe available!\nAdd items to your cart and checkout directly on the site.";
        }
        return "\u{1F44B} Welcome to Medga Store! How can I assist you with games, PlayStation Plus, Fortnite V-Bucks, Rocket League credits, or order tracking today? You can also message owner Selim on WhatsApp at 01042240852.";
      };
      if (!apiKey) {
...
1. PlayStation Games:
   ...
   - God of War Ragnar\xF6k (800 EGP)
